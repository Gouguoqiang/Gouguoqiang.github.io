...
> 详情见事务与锁
>
> 实战：对同一条数据的更新是高并发时（微服务多实例通过mq更新数据，但MQ是广播导致多实例并发）
>
> ​			唯一锁：如果获得锁的挂了这些更新就没了 落地实现库级可重入锁/redis等各种
>
> ​					库级可重入锁实现：lock表：host+thread作为获得锁的记录，用name作为资源名，time记录获得锁时间，每心跳时间去获取锁，host+thread与自己对不上则查询是否过去三倍时间来判断锁是否已被持有
>
> ​					库级乐观锁vertion字段， 查询和set原子化：会导致一些正常更新丢失
>
> ​			最终方案：通过业务主键查到目标记录，得到ID 用ID for update查询， 对字段进行比较有变动才进行更新
>
> 
>
